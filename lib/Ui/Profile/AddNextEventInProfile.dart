...
class AddNextEventInProfile extends StatefulWidget {
  @override
  _AddNextEventInProfileState createState() => _AddNextEventInProfileState();
}

class _AddNextEventInProfileState extends State<AddNextEventInProfile> {
  Timer _timer;
  ...
  void initState() {
    super.initState();
    _searchText = "";
    _selectedData.addAll(rq.nextEvents);
  ...
  void _callEventsApi() {
    ...
    RequestManager.postRequest(
        uri: endPoints.GetAllUpcomingEvent,
        body: body,
        isLoader: false,
                ...
                Padding(
                  ...
                  child: Container(
                      child: ButtonRegular(
                          ...
                          onPressed: () {
                            rq.nextEvents.clear();
                            rq.nextEvents.addAll(_selectedData);
                            Get.back();
                          })),
                )
              ],
                        ...
                        onTap: () {
                          if (!_selectedIds
                            ...
                            if (_selectedData.length < 20) {
                              //check for ticketmaster events
                              if (_list[index].eventId == 0) {
                                //create event in our DB and use
                                callCreateEvent(index);
                              } else {
                                _selectedIds
                                    .add(_list[index].eventId.toString());
                                _selectedData.add(_list[index]);
                              }
                            } else {
                              RequestManager.getSnackToast(
                                  title: LabelStr.lblLimitOver,
                                  message: Messages.C20Events);
                            }
                          } else {
                            _selectedIds
